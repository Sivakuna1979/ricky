...
const PRICING = [
  {
    name: 'Starter',      price: '£29',  period: '/month', highlight: false, badge: '',
    glow: '#3b82f6',  bg: '#07111f',
  ...
  {
    name: 'Enterprise',   price: '£199', period: '/month', highlight: false, badge: 'Best Value',
    glow: '#10b981',  bg: '#061510',
...
export default function HomePage() {
  return (
    <>
      {/* ─── Global styles: keyframes + responsive helpers ─── */}
      <style>{`
        *,*::before,*::after{box-sizing:border-box}
        ...
        /* Stats: 4-col on desktop, 2-col on tablet, 1-col on mobile */
      ...
      <main style={{ background: '#060914', minHeight: '100vh', overflowX: 'hidden', color: '#fff' }}>
        ...
        <section style={{ position: 'relative', minHeight: '94vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '100px 20px 80px', overflow: 'hidden', background: 'linear-gradient(160deg,#070918 0%,#0d1535 45%,#070918 100%)' }}>
          ...
          {(['🐟','🍔','☕','🍕','🍦','🥙'] as const).map((e, i) => {
            ...
            return (
              <div key={e} className="ft-float" style={{ position: 'absolute', fontSize: '2rem', opacity: .3, pointerEvents: 'none', animationDelay: `${i * 1.1}s`, ...pos }}>{e}</div>
            )
          })}
          ...
          <div style={{ position: 'relative', maxWidth: 860, width: '100%', margin: '0 auto', textAlign: 'center' }}>
            ...
            {/* live badge */}
            <div className="ft-pill-live">
              <span className="ft-dot" />
              <span>247 vans live near you right now</span>
            </div>

            {/* headline */}
            <h1 style={{ fontSize: 'clamp(2.5rem,7vw,5.2rem)', fontWeight: 900, lineHeight: 1.03, letterSpacing: '-0.035em', marginBottom: 28, color: '#fff', margin: '0 0 28px' }}>
              Find &amp; Order From<br />
              <span className="ft-shimmer">Mobile Food Vans</span><br />
              Near You
            </h1>
            ...
            <div className="ft-cat-grid">
              {CATEGORIES.map(c => (
                <Link key={c.type} href={`/search?type=${c.type}`} className="ft-card" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '28px 12px 22px', borderRadius: 18, textDecoration: 'none', background: c.bg, border: `1px solid ${c.border}`, boxShadow: `0 0 28px ${c.glow}14` }}>
                  {c.tag && (
                    <div style={{ position: 'absolute', top: 9, right: 9, fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 20, background: c.tag === 'Most Popular' ? '#f97316' : c.tag === 'Trending' ? '#ef4444' : '#10b981', color: '#fff' }}>{c.tag}</div>
                  )}
              ...
              {VANS.map(v => (
                <div key={v.name} className="ft-card" style={{ borderRadius: 22, overflow: 'hidden', background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', boxShadow: `0 0 40px ${v.glow}12` }}>
                  ...
                  <div style={{ height: 158, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `radial-gradient(ellipse at 30% 30%,${v.glow}30 0%,${v.bg} 70%)` }}>
                    ...
                    {/* rating */}
                    <div style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(8px)', borderRadius: 9, padding: '4px 10px', fontSize: 12, fontWeight: 700, color: '#fbbf24' }}>
                      ⭐ {v.rating} ({v.reviews})
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '20px 20px 22px' }}>
                    ...
                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', margin: '0 0 18px', display: 'flex', alignItems: 'center', gap: 5 }}>
                      <span>📍</span>{v.area}
                    </p>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      <Link href="/search" className="ft-btn-ghost" style={{ padding: '12px', borderRadius: 11, fontSize: 13 }}>
                        🗺️ Track
                      </Link>
                      <Link href="/search" className="ft-btn-primary" style={{ padding: '12px', borderRadius: 11, fontSize: 13 }}>
                        🛒 Order
                      </Link>
                    </div>
                  </div>
          ...
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            ...
            <Suspense fallback={
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 20 }}>
                {[0,1,2].map(i => <div key={i} style={{ height: 280, borderRadius: 20, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }} />)}
              </div>
            }>
              <FeaturedVans />
            </Suspense>
            ...
            <div className="ft-price-grid">
              {PRICING.map(plan => (
                <div key={plan.name} className="ft-card" style={{ position: 'relative', borderRadius: 24, padding: '36px 30px 34px', background: plan.bg, border: plan.highlight ? `1.5px solid ${plan.glow}55` : '1px solid rgba(255,255,255,.08)', boxShadow: plan.highlight ? `0 0 60px ${plan.glow}1a` : 'none' }}>
                  {plan.badge && (
                    <div style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: `linear-gradient(135deg,${plan.glow},${plan.glow}bb)`, color: '#fff', fontSize: 11, fontWeight: 800, padding: '5px 18px', borderRadius: 20, whiteSpace: 'nowrap', letterSpacing: '0.05em' }}>{plan.badge}</div>
                  )}
              ...
              {TESTIMONIALS.map(t => (
                <div key={t.name} className="ft-card" style={{ borderRadius: 24, padding: '32px', background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)' }}>
                  <div style={{ display: 'flex', gap: 3, marginBottom: 20 }}>
                    {[0,1,2,3,4].map(i => <span key={i} style={{ color: '#fbbf24', fontSize: 16 }}>★</span>)}
                  </div>
        ...
        <section style={{ padding: '100px 20px', position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg,#080e1e 0%,#152040 50%,#080e1e 100%)' }}>
          ...
          <div style={{ position: 'relative', maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, borderRadius: 9999, padding: '7px 18px', fontSize: 13, fontWeight: 600, marginBottom: 32, background: 'rgba(249,115,22,.1)', border: '1px solid rgba(249,115,22,.28)', color: '#fb923c' }}>
              🚀 14-day free trial — no credit card required
            </div>
            <h2 style={{ fontSize: 'clamp(2rem,6vw,3.5rem)', fontWeight: 900, color: '#fff', margin: '0 0 20px', lineHeight: 1.08, letterSpacing: '-0.03em' }}>
              Ready to Grow Your<br />Mobile Food Business?
            </h2>
            <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,.5)', lineHeight: 1.75, maxWidth: 540, margin: '0 auto 48px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, justifyContent: 'center' }}>
              <Link href="/register/business" className="ft-btn-primary" style={{ padding: '16px 36px', borderRadius: 14, fontSize: '1rem', boxShadow: '0 0 50px rgba(249,115,22,.35)', letterSpacing: '-0.01em' }}>
                Start Free Trial →
              </Link>
              <Link href="/search" className="ft-btn-ghost" style={{ padding: '16px 36px', borderRadius: 14, fontSize: '1rem' }}>
                Browse Vans
              </Link>
            </div>
